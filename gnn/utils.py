...
def eval_auc(label, score):
    ...
    score = torch.softmax(score, dim=1).cpu().detach().numpy()
    roc_auc = roc_auc_score(y_true=label, y_score=score, multi_class='ovr', average='macro')
    return roc_auc


def eval_acc(out, label):
    pred = out.argmax(dim=1)
    correct = (pred == label).sum()
    acc = int(correct) / len(label)
    return acc
